...
export class InventorySystem {
  ...
  static addItem(player: Player, itemId: string): void {
    player.inventory.push({ uid: String(nextUid++), itemId, upgradeLevel: 0 });
  }

  static equip(player: Player, uid: string): boolean {
  ...
  static unequip(player: Player, slot: ItemSlot): boolean {
  ...
  static upgrade(player: Player, slot: ItemSlot): UpgradeResult {
    const item = player.equipped[slot];
    if (!item) {
      return 'none';
    }
  ...
  static getCostFor(player: Player, slot: ItemSlot): number | null {
    const item = player.equipped[slot];
    return item ? upgradeCost(item) : null;
  }

  static describeEquipped(player: Player, slot: ItemSlot): string {
    const item = player.equipped[slot];
    if (!item) {
      return '(vacío)';
    }
